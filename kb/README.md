...
нужно сессии для работы, лежит здесь; тяжёлые бинарные профили выгружены в
`kb/raw/` (git-ignored), а одноразовые выводы дистиллированы в документы.
...
- [raw/](raw/.gitignore) — зеркала nsys-профилей (.sqlite, .nsys-rep),
  git-ignored: `.sqlite` запросимы без повторного GPU-прогона; сектор пуст после
  клона — это ожидаемо.
...
Сырые зеркала профилей — только в [raw/](raw/.gitignore).