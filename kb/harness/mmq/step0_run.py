#!/usr/bin/env python3
"""Step 0 driver: one ft serve boot (winner flags) under an nsys session.

Reuses .tasks/ft-gguf-serve-tuning/measure.py unchanged - only the FT binary
path is swapped for the ft_nsys.sh wrapper (nsys launch, deferred capture).
A watchdog thread tails the server log: FAILPAT -> hard kill; after the N-th
steady "input throughput" line -> nsys start / nsys stop to bound the capture
window to full prefill chunks (starts inside chunk 3, stops inside chunk 7).
"""
import argparse
import json
import os
...
measure.FT = os.path.join(TASKDIR, "ft_nsys.sh")
...
def main():
    logpath = os.path.join(TUNING, "logs", "step0_nsys.log")
