#!/usr/bin/env python3
"""v3a quality battery stage driver: census -> boot ft serve with the stage's
FREETOKEN_GGUF_MOE_MTILE -> /ready -> 24-prompt battery -> SIGTERM (30 s) ->
SIGKILL teardown -> census. Adapted from quality/run_battery.py (v2 env-flip);
the A/B here is baseline boot (no MTILE env, tile 4) vs candidate boot
(FREETOKEN_GGUF_MOE_MTILE=<tile>).

Usage: run_battery_v3a.py <base|win> [tile]
...
if STAGE not in ("base", "win"):
    sys.exit("usage: run_battery_v3a.py <base|win> [tile]")
MTILE = sys.argv[2] if len(sys.argv) > 2 else ""

ROOT = "/media/ai/src/FreeToken"
VDIR = os.path.join(ROOT, ".tasks/mmq-v3-stationary-moe")
QDIR = os.path.join(VDIR, "quality")
...
# harness-side probe: surfaces the bare-logger one-shot marker
PYPATH = os.path.join(ROOT, ".tasks/mmq-prefill-kernel/probe_sitecustomize")
...
def boot():
    ...
    for p in census("pre-boot"):
        ...
        time.sleep(5)
    env = dict(os.environ)
    env.pop("FREETOKEN_GGUF_MOE_MTILE", None)
    env.pop("FREETOKEN_GGUF_GROUPED_PREFILL", None)
    if STAGE == "win":
        env["FREETOKEN_GGUF_MOE_MTILE"] = MTILE
...
def boot_facts(ready_s):
    ...
    facts = {"ready_s": ready_s, "failpat": state["failpat"],
             "mtile_env": ("win" if STAGE == "win" else "unset")}
    for pat, name in [
...
def main():
    rc = 0
    summary = {"stage": STAGE, "env": ("FREETOKEN_GGUF_MOE_MTILE=" + MTILE if STAGE == "win" else "unset"),
    ...
    try:
        ...
        if ready_s is None:
            ...
            cmd = [sys.executable, os.path.join(ROOT, ".tasks/mmq-prefill-kernel/quality/ft_phase6_battery.py"),
