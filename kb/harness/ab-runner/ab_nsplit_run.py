#!/usr/bin/env python3
"""N-split A/B driver (dense-q80-gemm replan wave 1): one boot per NSPLIT.

Single variable: FREETOKEN_GGUF_KDA_NSPLIT (nsplit1: unset -> 1, nsplit2: "2").
MTILE=32 anchor fixed on both. measure.py reused unchanged except FT swapped
for the nsys wrapper (D09: launch + start-after-ready + --cuda-graph-trace=node).
...
# Greedy prompts for the cross-boot bitwise diff (mix of lengths/topics,
# two digit prompts); identical order/content/settings in both boots.
PROMPTS = [
...
def do_prompts(logpath):
    ...
    for i, content in enumerate(PROMPTS):
        rq_path = os.path.join(TASKDIR, "req_ns_%d.json" % i)
...
def main():
    ...
    ap.add_argument("--name", required=True, choices=["nsplit1", "nsplit2"])
    a = ap.parse_args()
    name = a.name
    session = name + "cap"
    ...
    if name == "nsplit2":
        env_pairs.append("FREETOKEN_GGUF_KDA_NSPLIT=2")
        os.environ["FREETOKEN_GGUF_KDA_NSPLIT"] = "2"
    else:
        # unset = knob value 1 (the boot-1 side); never inherit from the shell
