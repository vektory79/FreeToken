...
class GraphRunner:
    def __init__(
        ...
        if moe_offload_cache is None:
            self.moe_offload_caches: list[OffloadMoeCache] = []
        elif isinstance(moe_offload_cache, (list, tuple)):
            self.moe_offload_caches = list(moe_offload_cache)
        else:
            # Single cache: duck-typed on purpose, test doubles are not OffloadMoeCache.
            self.moe_offload_caches = [moe_offload_cache]
        self.mrope = mrope
        self.stream = stream
        self.device = device
